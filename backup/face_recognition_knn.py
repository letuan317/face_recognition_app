...
def predict(X_img_path, knn_clf=None, model_path=None, distance_threshold=0.6):
    ...
    return [(pred, loc, percent) if rec else ("unknown", loc, percent) for pred, loc, rec in zip(knn_clf.predict(faces_encodings), X_face_locations, are_matches)]
...
if __name__ == "__main__":
    ...
    #classifier = train("data/people", model_save_path="data/trained_knn_model.clf", n_neighbors=3)
    print("Training complete!")

    # STEP 2: Using the trained classifier, make predictions for unknown images
    for image_file in os.listdir("data/test"):
        full_file_path = os.path.join("data/test", image_file)
        ...
        predictions = predict(
            full_file_path, model_path="data/trained_knn_model.clf")
        # print(predictions)
        # Print results on the console
        if(len(predictions) > 0):
            for name, (top, right, bottom, left), percent in predictions:
                print("- Found {} at ({}, {}) {}".format(name, left, top, percent))
        ...
        # show_prediction_labels_on_image(os.path.join("data/test", image_file), predictions)
        tss = 0
        img = cv2.imread(full_file_path)
        # Draw a label with a name below the face
        cv2.rectangle(img, (left, bottom - 35),
                      (right, bottom), (52, 235, 70), cv2.FILLED)
        font = cv2.FONT_HERSHEY_DUPLEX
        cv2.putText(img, str(percent), (left + 6, bottom - 6),
                    font, 1.0, (255, 255, 255), 1)
        while True:
            new_file_name = 'data/knn_predicted/' + \
                str(name)+'_'+str(percent)+'_'+str(tss)+'.jpg'
